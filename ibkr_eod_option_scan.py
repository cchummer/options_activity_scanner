"""
8/2026 — IBKR + SEC EOD Option Scan
=================================================================
...
class IBKRApp(EWrapper, EClient):
    ...
    def __init__(self):
        ...
        self._last_errors: dict[int, tuple[int, str]] = {}  # reqId -> (errorCode, errorString)
    ...
    def error(self, reqId, errorTime, errorCode, errorString, advancedOrderRejectJson=""):
        ...
        if reqId > 0:
            with self._lock:
                slot = self._req_store.get(reqId)
                if slot is not None:
                    slot["error"] = (errorCode, errorString)
            self._last_errors[reqId] = (errorCode, errorString)
            self._signal(reqId)

    def pop_req_error(self, req_id: int):
        slot = self._get(req_id)
        return slot.get("error")

    @staticmethod
    def is_pacing_or_limit_error(error_code: int, error_text: str = "") -> bool:
        pacing_codes = {
            100,  # max rate of messages per second exceeded
            101,  # max number of tickers reached
            162,  # historical data service error (often pacing text)
            321,  # validation/server error (sometimes pacing-like)
            354,  # not subscribed
            420,  # invalid real-time query / pacing-like variants
        }
        if error_code in pacing_codes:
            return True
        t = (error_text or "").lower()
        return any(k in t for k in [
            "pacing", "rate", "too many", "max number",
            "market data lines", "exceeded", "historical data request", "throttl"
        ])
...
class SimpleRateLimiter:
    ...
    Blocks until max_calls within period_sec is respected.
    """
    ...
    def wait(self):
        while True:
            now = time.time()
            with self._lock:
                while self._events and (now - self._events[0]) > self.period_sec:
                    self._events.popleft()
                if len(self._events) < self.max_calls:
                    self._events.append(now)
                    return
                sleep_for = self.period_sec - (now - self._events[0])
...
class Pillar1MarketData:
    ...
    QUALIFY_DELAY    = 0.15   # seconds: pacing pause between qualifications
    ...
    def __init__(self, host='127.0.0.1', port=4001, client_id=1): # TWS Port=7496, Paper=7497, Live GW=4001, Paper GW=4002
        ...
        # conservative defaults, tune upward after stable runs
        self._lim_contract = SimpleRateLimiter(max_calls=8,  period_sec=1.0)
        self._lim_hist     = SimpleRateLimiter(max_calls=4,  period_sec=1.0)
        self._lim_secdef   = SimpleRateLimiter(max_calls=3,  period_sec=1.0)
        self._lim_mkt_open = SimpleRateLimiter(max_calls=10, period_sec=1.0)

        self._active_mkt_lines = 0
        self._active_mkt_lock = threading.Lock()
        self.MAX_ACTIVE_MKT_LINES_BUDGET = 80  # adjust to your entitlement
    ...
    def _acquire_line_budget(self):
        while True:
            with self._active_mkt_lock:
                if self._active_mkt_lines < self.MAX_ACTIVE_MKT_LINES_BUDGET:
                    self._active_mkt_lines += 1
                    return
            time.sleep(0.05)

    def _release_line_budget(self):
        with self._active_mkt_lock:
            self._active_mkt_lines = max(0, self._active_mkt_lines - 1)

    def _request_with_backoff(self, do_request, req_id: int, wait_timeout: float, retries: int = 3):
        """
        Send request + wait, retrying with exponential backoff on pacing/limit errors.
        do_request: zero-arg callable that sends the IB request.
        """
        for attempt in range(retries):
            do_request()
            ok = self._app._wait(req_id, timeout=wait_timeout)
            err = self._app.pop_req_error(req_id)
            ...
            if err:
                code, text = err
                if self._app.is_pacing_or_limit_error(code, text) and attempt < retries - 1:
                    backoff = (1.25 * (2 ** attempt)) + random.uniform(0.0, 0.35)
                    logging.warning(
                        f"Req {req_id} pacing/limit error ({code}: {text}). "
                        f"Retrying in {backoff:.2f}s (attempt {attempt+1}/{retries})"
                    )
                    time.sleep(backoff)
                    self._app._alloc(req_id, rows=[], ticks={})  # reset slot/event/error
                    continue
            return False
        return False
    ...
    def _qualify_one(self, contract: Contract) -> Contract | None:
        ...
        self._lim_contract.wait()
        ok = self._request_with_backoff(
            do_request=lambda: self._app.reqContractDetails(rid, contract),
            req_id=rid,
            wait_timeout=self.QUALIFY_TIMEOUT,
            retries=3
        )
    ...
    def _qualify_many(self, contracts: list) -> tuple[list, dict]:
        ...
        for c in contracts:
            ...
            self._lim_contract.wait()
            ok = self._request_with_backoff(
                do_request=lambda c=c, rid=rid: self._app.reqContractDetails(rid, c),
                req_id=rid,
                wait_timeout=self.QUALIFY_TIMEOUT,
                retries=3
            )
    ...
    def _get_historical_bars(self, contract: Contract,
        ...
        def _send_hist():
            self._app.reqHistoricalData(
                rid, contract,
                '', duration, bar_size, what_to_show,
                1, 1, False, []
            )

        ok = self._request_with_backoff(
            do_request=_send_hist,
            req_id=rid,
            wait_timeout=self.HIST_TIMEOUT,
            retries=3
        )

        if not ok:
            logging.warning(
                f"Historical data timed out/failed for {getattr(contract, 'symbol', '?')} "
    ...
    def fetch_ticker_snapshots(self, contracts: list) -> list:
        ...
        for contract in contracts:
            ...
            self._lim_mkt_open.wait()
            self._acquire_line_budget()
        ...
        for rid, ticker in rid_to_ticker.items():
            ...
            avg_vol  = _safe_int(ticks.get(TICK_AVG_OPT_VOL), default=1) or 1
            ...
            if opt_vol > avg_vol and opt_vol >= 1000:
                logging.info(f"{ticker.contract.symbol} passed: Option Vol {opt_vol} (> Avg {avg_vol})")
                valid_candidates.append(ticker)
            else:
                logging.info(f"{ticker.contract.symbol} filtered: opt_vol={opt_vol}, avg={avg_vol}")
    ...
    def safe_fetch_tickers(self, contracts: list, chunk_size: int = 40) -> list:
        ...
        chunk_size = min(chunk_size, 30)  # safer cap
        ...
        for chunk_start in range(0, len(valid_contracts), chunk_size):
            ...
            try:
                # Pass 1: live streaming for OI + IV
                ...
                # Pass 2: frozen for contract volume
                ...
                for ticker in chunk_tickers:
                    ...
                    self._lim_mkt_open.wait()
                    self._acquire_line_budget()
                    self._app.reqMktData(rid, ticker.contract, '100,106', False, False, [])
    ...
    def analyze_positioning_footprint(self, contract: Contract,
        ...
        try:
            ...
            ok = self._request_with_backoff(
                do_request=_send_secdef,
                req_id=rid,
                wait_timeout=self.SCANNER_TIMEOUT,
                retries=3
            )

            if not ok:
                logging.warning(f"Option chain param request timed out/failed for {contract.symbol}")
                return _err
...
class Pillar2SECData:
    ...
    def extract_sic_industry(symbol):
        """
        Pull SIC metadata for a given stock symbol using the sec-edgar-downloader library.
        Returns:
            {
